...
foreach $file ( @files ) {
		...
		$output = `less $file | jq '.tracklist[] | {title}' | grep title | sed 's/"title": /|/g' |  sed -e 's/^[ \\t]*//' | nl -n rn -s '' | sed -e "s/.*/$releaseID:TRACKNAME:&/"`;
		chomp($output);
		print FH "$output\n";
        $output = `less $file | jq '.tracklist[] | {position}' | grep position | sed 's/"position": /|/g' |  sed -e 's/^[ \\t]*//' | nl -n rn -s '' | sed -e "s/.*/$releaseID:BAND:&/"`;
		chomp($output);
		print FH "$output\n";
		if ($variousArtist != 0) {
            $output = `less $file | jq '.tracklist[] | {artists}' | grep name | sed 's/"name": /|/g' |  sed -e 's/^[ \\t]*//' | nl -n rn -s '' | sed -e "s/.*/$releaseID:ARTIST:&/"`;
		    chomp($output);
		    print FH "$output\n";
		}
